...
function getFinalScoreLastToWin(
  ...
  // Llevamos la cuenta de que boards ya no comprobar porque ya ganaron
  let winningBoards: number[] = new Array(n).fill(0);
  // Cuando todas ganaron dejamos de leer instrucciones
  let endingCondition = () => winningBoards.every(Boolean);
  ...
  loop0: for (const [index, instruction] of instructions.entries()) {
    ...
    // 4. Recorremos todas las tablas y marcamos segun numero de instruction
    // loop1: for (const [i, board] of boards.entries()) {
    for (let i = 0; i < boards.length; i++) {
      if (winningBoards[i]) continue;
      // console.log({ i });
      for (const [j, row] of boards[i].entries()) {
        for (const [k, value] of row.entries()) {
          let keyNumber: number = parseInt(Object.keys(value)[0]);
          // let isMarked = value[keyNumber];
          // console.log(`-> key: ${keyNumber} isMarked: ${isMarked}`);
          if (instruction === keyNumber) {
            ...
            for (const [indexGanador, board] of boards.entries()) {
              ...
              if (conditionToBreak) {
                ...
                winningIndex = indexGanador;
                winningIndication = instruction;
                winningBoards[winningIndex] = 1;
                if (endingCondition()) break loop0;
              }
  ...
  // console.dir(boards[winningIndex], { depth: 5 });
  ...
  result = sum * winningIndication;
  // console.log(
  //   `-> sum * winningIndication: ${sum} * ${winningIndication} result is ${result}`
  // );
...
export { getFinalScore, getFinalScoreLastToWin };
