import { getTotalPoints, getTotalScratchcards } from "../04";
...
Card 6: 31 18 13 56 72 | 74 77 10 23 35 67 36 11`,
  ...
  [17803, readInputFromFile("04")],
];

test.each(listOfEntries)("results in %i", (expected, entries) => {
  expect(getTotalPoints(entries)).toBe(expected);
});
...
Card 6: 31 18 13 56 72 | 74 77 10 23 35 67 36 11`,
  ...
  [5554894, readInputFromFile("04")],
];

test.each(listOfEntries2)("results in %i", (expected, entries) => {
  expect(getTotalScratchcards(entries)).toBe(expected);
});
